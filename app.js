...
(function() {
  ...
  function launchAgent(target, source, question, copyOnly){
    if (target === 'local') {
      if (source === 'home') {
        var terminal = g('home-ai-terminal');
        if (terminal) {
          terminal.innerHTML = "";
          terminal.textContent = "Retrieving PMN context and compiling synthesis...";
          var pack = buildContextPack(question, {
            fallbackIds: ['how-to-read-this-document','3.0','6.1','10.1','15.15','17.0'],
            maxSections: 5,
            maxGlossary: 6
          });
          callClaude([{role:'user',content:question}], SYS, function(text){
            terminal.innerHTML = text.replace(/\n/g, '<br>').replace(/█/g, '<span class="terminal-cursor"></span>');
          });
        }
      }
      return;
...
(function(){
  ...
  (function(){
    ...
    results.addEventListener('click', function(e){
      ...
      if (action === 'theme') {
        ...
        var pi = Math.floor(Math.random() * PARTS.length);
        var si = Math.floor(Math.random() * PARTS[pi].subs.length);
        jumpToSection(pi, si, { origin: getJumpOrigin() });
        hide();
        ...
        jumpToSectionById(id, { origin: getJumpOrigin() });
        hide();
      }
    });
  })();
  ...
  (function(){
    ...
    document.addEventListener('mouseover', function(e){
      ...
      var sec = PARTS[info.pi].subs[info.si];
      var plain = stripHtml(sec.html || sec.text || '');
      var excerpt = plain.slice(0, 180) + (plain.length > 180 ? '...' : '');
      
      card.innerHTML = 
        '<span class="xref-preview-kicker">Part ' + PARTS[info.pi].part + ' &mdash; Section ' + sid + '</span>'
        + '<h4 class="xref-preview-title">' + esc(sec.title) + '</h4>'
        + '<p class="xref-preview-excerpt">' + esc(excerpt) + '</p>'
