...
(function initializeApp() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!hasInlined) {
            ...
            yield Promise.all(dataFiles.map((file) => __awaiter(this, void 0, void 0, function* () {
                try {
                    const res = yield fetch(`data/${file}.json`);
                    if (res.ok)
                        mockData[`d-${file}`] = yield res.text();
                }
                catch (e) {
        ...
        (function () {
            ...
            try {
                readMap = JSON.parse(localStorage.getItem('pmn-read') || '{}');
            }
            catch (e) { }
            function g(id) { return document.getElementById(id); }
            function esc(s) { return String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }
            function isR(p, s) { return !!readMap[p + '-' + s]; }
            function markR(p, s) { readMap[p + '-' + s] = true; try {
                localStorage.setItem('pmn-read', JSON.stringify(readMap));
            }
            catch (e) { } pip(); }
            function savePos(p, s) { try {
                localStorage.setItem('pmn-pos', p + ',' + s);
            }
            catch (e) { } }
            function loadPos() { try {
            ...
            function displaySectionId(id) {
                var sid = String(id || '');
                if (SECTION_LABELS[sid])
                    return SECTION_LABELS[sid];
                return sid;
            }
            // Theme
            function applyTheme(t) {
                ...
                var tog = g('theme-tog');
                if (tog)
                    tog.textContent = (t === 'dark' ? '☀ Light' : '☾ Dark');
            }
            ...
            function applyLayout(mode) {
                ...
                try {
                    if (mode)
                        localStorage.setItem('pmn-layout', mode);
                    else
            ...
            function applyReaderScale(scale) {
                ...
                ['font-sm', 'font-md', 'font-lg'].forEach(function (id) {
                    var btn = g(id);
                    if (btn)
                        btn.classList.toggle('on', id === ids[scale]);
                });
            }
            ...
            function applyReaderMeasure(measure) {
                ...
                ['measure-narrow', 'measure-base', 'measure-wide'].forEach(function (id) {
                    var btn = g(id);
                    if (btn)
                        btn.classList.toggle('on', id === ids[measure]);
                });
            }
            ...
            function updateReaderAgentStatus() {
                ...
                if (!buildCtx)
                    return; // Lewati jika modul AI belum siap
                var pack = buildCtx('', {
                    activeId: s.id,
            ...
            function injectReadingPaths() {
                if (document.querySelector('.reading-paths'))
                    return;
            ...
            function upgradeHomeAi() {
                ...
                if (!inner || g('home-ai-copy') || document.getElementById('hai-tabs'))
                    return;
                inner.innerHTML =
            ...
            function pip() {
                ...
                for (var i = 0; i < PARTS.length; i++) {
                    ...
                    for (var j = 0; j < ps.length; j++) {
                        tot++;
                        if (ps[j].subs)
                            for (var k = 0; k < ps[j].subs.length; k++) {
                                tot++;
                                if (ps[j].subs[k].subs)
                                    tot += ps[j].subs[k].subs.length;
                            }
                    }
                }
                ...
                if (g('pip-num'))
                    g('pip-num').textContent = done + '/' + tot;
                g('st-p').textContent = PARTS.length;
                g('st-s').textContent = tot;
                g('st-r').textContent = pct + '%';
                if (g('toc-stat-parts'))
                    g('toc-stat-parts').textContent = PARTS.length;
                if (g('toc-stat-sections'))
                    g('toc-stat-sections').textContent = tot;
                if (g('toc-stat-read'))
                    g('toc-stat-read').textContent = pct + '%';
            ...
            function nav(v) {
                ...
                if (typeof updateMobNav === 'function')
                    updateMobNav();
            }
            function showContentsPanel() {
                ...
                var hdr = g('sv-hdr');
                if (hdr)
                    hdr.textContent = 'Contents — Manuscript Map';
                var si = g('srch-in');
                if (si)
                    si.value = '';
                var sc = g('srch-clr');
            ...
            function getSectionMeta(pi, si) {
                if (typeof pi !== 'number' || typeof si !== 'number' || !PARTS[pi] || !PARTS[pi].subs[si])
                    return null;
            ...
            function getJumpOrigin() {
                ...
                if (view === 'reader')
                    return getSectionMeta(curP, curS);
                if (view === 'srch') {
                    var q = g('srch-in') ? g('srch-in').value.trim() : '';
                    var tocPanel = g('toc-panel');
            ...
            function resolveSectionId(sid) {
                if (LOOK[sid])
                    return sid;
                if (LEGACY_XREF_IDS[sid] && LOOK[LEGACY_XREF_IDS[sid]])
                    return LEGACY_XREF_IDS[sid];
                return sid;
            }
            ...
            function linkXrefs(container) {
                ...
                function walk(node) {
                    ...
                    if (tag === 'SCRIPT' || tag === 'STYLE' || tag === 'A' || tag === 'MARK')
                        return;
            ...
            function openSec(pi, si, nohash) {
                ...
                if (meta.origin && !(meta.origin.kind === 'section' && meta.origin.pi === pi && meta.origin.si === si))
                    xrefBack = meta.origin;
                else if (meta.trackBack && prev && !(prev.pi === pi && prev.si === si))
                    xrefBack = prev;
                else if (!meta.keepBack)
                    xrefBack = null;
                curP = pi;
                curS = si;
                markR(pi, si);
                ...
                renderSec();
                nav('reader');
                if (typeof window._afterOpenSec === 'function')
                    window._afterOpenSec(pi, si);
                if (!nohash) {
                    try {
                        history.pushState(null, '', '#' + PARTS[pi].subs[si].id);
                    }
                    catch (e) { }
                }
                if (window.innerWidth <= 680) {
                ...
                updateJumpBack();
                setTimeout(function () { var a = g('sb-list').querySelector('.sb-item.on'); if (a)
                    a.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); }, 60);
            }
            ...
            function saveHomeNotes() {
                ...
                try {
                    if (ta.value.trim())
                        localStorage.setItem('pmn-home-notes', ta.value);
                    else
            ...
            function updateHomeSnapshot(partsCount, sectionCount, readPct) {
                ...
                if (ver)
                    ver.textContent = getPublicVersionTag();
                if (pcs)
                    pcs.textContent = String(partsCount);
                if (secs)
                    secs.textContent = String(sectionCount);
                if (prog)
                    prog.textContent = readPct + '%';
            }
            // ── Export
            function exportSec() {
                var s = PARTS[curP].subs[curS];
            ...
            function copyText(text, onSuccess) {
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard.writeText(text).then(function () { if (onSuccess)
            ...
            function openAnnotSummary() {
                ...
                for (var pi = 0; pi < PARTS.length; pi++) {
                    for (var si = 0; si < PARTS[pi].subs.length; si++) {
                        ...
                        if (note.trim())
                            entries.push({ id: s.id, title: s.title, note: note.trim() });
                    }
                }
            ...
            function renderSec() {
                ...
                if (g('read-time'))
                    g('read-time').textContent = mins + ' min read';
                ...
                if (fl)
                    fl.setAttribute('href', 'mailto:novadharma.pm@gmail.com?subject=' + subj + '&body=' + body);
                var tweet = encodeURIComponent('Re: PMN Reader - ' + s.id + ' ' + s.title + ' @nova_dharma_pm');
                var tl = g('twitter-link');
                if (tl)
                    tl.setAttribute('href', 'https://twitter.com/intent/tweet?text=' + tweet);
            }
            function nextSec(pi, si) {
                si++;
                if (si >= PARTS[pi].subs.length) {
                    pi++;
                    si = 0;
                }
                if (pi >= PARTS.length)
                    return;
                if (PARTS[pi].subs[si] && PARTS[pi].subs[si].is_intro) {
                    si++;
                    if (si >= PARTS[pi].subs.length) {
                        pi++;
                        si = 0;
            ...
            function updateMobNav() {
                ...
                if (pp)
                    pp.disabled = (curP === 0 && curS === 0);
                if (nn)
                    nn.disabled = (curP === PARTS.length - 1 && curS === PARTS[curP].subs.length - 1);
            }
            var _st = null;
            function onSI(v) {
                ...
                if (v.length >= 3) {
                    nav('srch');
                    _st = setTimeout(function () { doSrch(v); }, 180);
                }
                else if (!v) {
                    nav('home');
                }
            ...
            function onSF() { var v = g('srch-in').value; if (v.length >= 3) {
                nav('srch');
                hideContentsPanel();
                doSrch(v);
            }
            ...
            function doSrch(q) {
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    if (k.toLowerCase() === ql || ql.indexOf(k.toLowerCase()) >= 0 || k.toLowerCase().indexOf(ql) >= 0) {
                        ...
                        while ((match = rx.exec(def)) !== null) {
                            var secId = match[1];
                            if (citedSections.indexOf(secId) < 0)
                                citedSections.push(secId);
                        }
                    }
                }
                ...
                for (var pi = 0; pi < PARTS.length; pi++) {
                    ...
                    for (var si = 0; si < p.subs.length; si++) {
                        ...
                        var key = pi + '-' + si;
                        if (addedSections[key])
                            continue; // Already added as glossary match
                ...
                function hl(t) { if (!q || q.length < 2)
                    return esc(t); return esc(t).replace(new RegExp('(' + q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ')', 'gi'), '<mark>$1</mark>'); }
                ...
                if (!res.length)
                    h = '<p class="empty">No results found.</p>';
                g('sv-body').innerHTML = h;
                // clicks handled by delegation
            }
            ...
            g('sv-body').addEventListener('click', function (e) {
                ...
                if (g('srch-view').classList.contains('on'))
                    clrS();
            });
            ...
            g('toc-panel').addEventListener('click', function (e) {
                ...
                if (sid) {
                    var resolvedId = resolveSectionId(sid);
                    if (LOOK[resolvedId])
                        jumpToSection(LOOK[resolvedId].pi, LOOK[resolvedId].si, { origin: getJumpOrigin() });
                    return;
                }
                if (part)
                    openPartStart(part, { origin: getJumpOrigin() });
            });
        ...
        (function () {
            ...
            display.forEach(function (part, i) {
                ...
                if (part.subs.length > 8)
                    secList += '<span style="font-family:var(--f-mono);font-size:.65rem;color:var(--mute)">+ ' + (part.subs.length - 8) + ' more sections</span>';
  ...
  `;
        ...
        (function () {
            ...
            function ensureData() {
                if (!PARTS) {
                    try {
                        PARTS = JSON.parse(document.getElementById('d-parts').textContent);
                    }
                    catch (e) {
                        PARTS = [];
                    }
            ...
            function buildContextPack(query, opts) {
                ...
                for (var pi = 0; pi < PARTS.length; pi++) {
                    ...
                    for (var si = 0; si < p.subs.length; si++) {
                        ...
                        if (q.length > 2 && blob.indexOf(q) >= 0)
                            score += 3;
                        q.split(/\s+/).filter(function (w) { return w.length > 3; }).forEach(function (w) { if (blob.indexOf(w) >= 0)
                            score++; });
                        if (opts.activeId && s.id === opts.activeId)
                            score += 5;
                        if (opts.relatedIds && opts.relatedIds.indexOf(s.id) >= 0)
                            score += 4;
                        if (opts.fallbackIds && opts.fallbackIds.indexOf(s.id) >= 0)
                            score += 2;
                        if (score > 0)
                            scored.push({ pi: pi, si: si, s: s, p: p, score: score });
                    }
                }
            ...
            (function () {
                ...
                function switchTab(mode) {
                    ...
                    Object.keys(panels).forEach(function (k) { if (panels[k])
                ...
                if (homeChatgptSwitch)
                    homeChatgptSwitch.addEventListener('click', function () {
                        switchTab('chatgpt');
                ...
                if (homeGeminiSwitch)
                    homeGeminiSwitch.addEventListener('click', function () {
                        switchTab('gemini');
                ...
                if (inp)
                    inp.addEventListener('keypress', function (e) { if (e.key === 'Enter') {
                        e.preventDefault();
                        ask();
                    } });
                if (clearBtn)
                    clearBtn.addEventListener('click', function () {
                        history = [];
                        if (chatLog)
                            chatLog.innerHTML = '<div class="hcl-placeholder">&gt;&gt; STATIC BUILD DETECTED — inline provider is parked until a secure backend is connected.<br>&gt;&gt; Use ChatGPT/Gemini tabs for grounded handoff, or open the PMN Agent Guide below.</div>';
                    });
                if (exportBtn)
                ...
                function setupRedirect(qId, btnId, cpyId, statId, modeId, url) {
                    ...
                    function go(openWin) {
                        ...
                        navigator.clipboard.writeText(full).then(function () {
                            ...
                            if (qEl)
                                qEl.value = '';
                        }).catch(function () { if (stat)
                            stat.textContent = 'Clipboard blocked. Try copy-only.'; });
                        if (openWin && !win && stat)
                            stat.textContent = 'Pop-up blocked. Open the site manually and paste.';
                    }
                    rbtn.addEventListener('click', function () { go(true); });
                    if (cpyBtn)
                        cpyBtn.addEventListener('click', function () { go(false); });
                    if (qEl)
                        qEl.addEventListener('keypress', function (e) { if (e.key === 'Enter') {
                            e.preventDefault();
                            go(true);
                        } });
                }
            ...
            (function () {
                ...
                if (readerChatgptSwitch)
                    readerChatgptSwitch.addEventListener('click', function () {
                        switchTab('chatgpt');
                        var q = document.getElementById('reader-chatgpt-q');
                        if (q)
                            q.focus();
                    });
                if (readerGeminiSwitch)
                    readerGeminiSwitch.addEventListener('click', function () {
                        switchTab('gemini');
                ...
                if (chatIn)
                    chatIn.addEventListener('keypress', function (e) { if (e.key === 'Enter') {
                        e.preventDefault();
                        ask();
                    } });
                if (clearBtn)
                    clearBtn.addEventListener('click', function () {
                        rHistory = [];
                        if (chatLog)
                            chatLog.innerHTML = '&gt;&gt; INLINE BACKEND MODE IS NOT LIVE IN THIS STATIC BUILD.<br>&gt;&gt; Use ChatGPT/Gemini handoff for grounded answers, or connect a secure backend for in-page replies.';
                    });
                ...
                function setupReaderRedirect(qId, btnId, cpyId, statId, modeId, url) {
                    ...
                    function go(openWin) {
                        ...
                        navigator.clipboard.writeText(full).then(function () {
                            ...
                            stat.textContent = 'Clipboard blocked.'; });
                        if (openWin && !win && stat)
                            stat.textContent = 'Pop-up blocked. Open manually and paste.';
                    }
                    rbtn.addEventListener('click', function () { go(true); });
                    if (cpyBtn)
                        cpyBtn.addEventListener('click', function () { go(false); });
                ...
                window._pmnClearReaderChat = function () { rHistory = []; if (chatLog)
                    chatLog.innerHTML = ''; };
            })();
            // ──────────────────────────────────────────────────────────────────
            ...
            (function () {
                ...
                document.addEventListener('mouseup', function (e) {
                    ...
                    if (!prose || !prose.contains(e.target)) {
                        setTimeout(function () { var sel = window.getSelection(); if (!sel || sel.isCollapsed)
                            hideToolbar(); }, 100);
                        return;
                    }
                    setTimeout(function () {
                ...
                toolbar.querySelectorAll('.hl-btn').forEach(function (btn) {
                    btn.addEventListener('click', function (e) {
                        ...
                        if (editHlId) {
                            ...
                            if (sp)
                                sp.className = 'pmn-hl' + (color !== 'red' ? ' hl-' + color : '');
                            hideToolbar();
                        }
                        else {
                            applyHL(color, note);
                ...
                document.getElementById('hl-save-note-btn').addEventListener('click', function (e) {
                    ...
                    if (editHlId && secId) {
                        ...
                        var sp = document.querySelector('[data-hl-id="' + editHlId + '"]');
                        if (sp)
                            sp.title = note ? '📝 ' + note : 'Click to annotate';
                        hideToolbar();
                    }
                    else if (curSel) {
                        applyHL('red', note);
                ...
                document.getElementById('hl-remove-btn').addEventListener('click', function (e) {
                    ...
                    if (editHlId && secId) {
                        ...
                        if (sp) {
                            ...
                            while (sp.firstChild)
                                par.insertBefore(sp.firstChild, sp);
                            par.removeChild(sp);
                        }
                        updateBadge(secId);
                    }
                ...
                function doSemantic(q) {
                    ...
                    for (var pi = 0; pi < PARTS.length; pi++) {
                        ...
                        for (var si = 0; si < p.subs.length; si++) {
                            ...
                            Object.keys(GL || {}).forEach(function (term) {
                                if (q.toLowerCase().indexOf(term.toLowerCase()) >= 0 && blob.indexOf(term.toLowerCase()) >= 0)
                                    score += 3;
                            });
                            if (score > 0)
                                scored.push({ pi: pi, si: si, s: s, p: p, score: score });
                ...
                if (typeof window.openSec === 'function' && !window._pmnOpenSecPatched) {
                    ...
                    window.openSec = function (pi, si, nohash) {
                        ...
                        setTimeout(function () {
                            ...
                            if (s) {
                                if (window._pmnRenderHL)
                                    window._pmnRenderHL(s.id);
                                if (window._pmnClearReaderChat)
                                    window._pmnClearReaderChat();
                            }
                        }, 80);
                    };
                }
        ...
        (function () {
            ...
            if (readerMain)
                readerMain.addEventListener('scroll', updateProgress, { passive: true });
            // 2. Focus Mode Toggle
            ...
            if (focusBtn) {
                focusBtn.addEventListener('click', function () {
                    ...
                    setTimeout(() => { if (typeof resize === 'function')
                        resize(); }, 300);
                });
            }
            ...
            function injectGlossaryTooltips() {
                ...
                prose.querySelectorAll('p').forEach(p => {
                    if (p.hasAttribute('data-glossary-processed'))
                        return;
            ...
            function injectCopyAnchors() {
                ...
                prose.querySelectorAll('h2[id], h3[id], h4[id]').forEach(heading => {
                    if (heading.querySelector('.copy-anchor'))
                        return;
            ...
            (function () {
                ...
                function renderDefault() {
                    var h = '<button class="cmd-palette-item" data-action="theme">'
            ...
            const proseObserver = new MutationObserver((mutations) => {
                ...
                mutations.forEach(m => { if (m.addedNodes.length > 0)
                    shouldUpdate = true; });
                if (shouldUpdate) {
                    injectCopyAnchors();
            ...
            if (proseEl)
                proseObserver.observe(proseEl, { childList: true, subtree: false });
            ...
            (function () {
                ...
                if (startBtn) {
                    startBtn.addEventListener('click', function (e) {
                        ...
                        if (window.nav)
                            window.nav('srch');
                        if (window.showContentsPanel)
                            window.showContentsPanel();
                    });
                }
