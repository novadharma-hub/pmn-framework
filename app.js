(async function initializeApp() {
  ...
  (function() {
    var PARTS2 = JSON.parse(document.getElementById("d-parts").textContent);
    ...
    var curP = 0, curS = 0, sbOpen = true, readMap2 = {}, readerScale = 1;
    try {
      readMap2 = JSON.parse(localStorage.getItem("pmn-read") || "{}");
    } catch (e) {
    }
    function g(id) {
    ...
    function markR(p, s) {
      readMap2[p + "-" + s] = true;
      try {
        localStorage.setItem("pmn-read", JSON.stringify(readMap2));
      } catch (e) {
      }
      pip2();
    }
    function savePos(p, s) {
      try {
        localStorage.setItem("pmn-pos", p + "," + s);
      } catch (e) {
      }
    }
    function loadPos() {
      try {
        var v = localStorage.getItem("pmn-pos");
        if (v) {
          var a = v.split(",");
          return [+a[0], +a[1]];
        }
    ...
    function pshort(p) {
      return SPECIAL[p.part] ? p.title : "Part " + p.part;
    }
    function plong(p) {
      return SPECIAL[p.part] ? "" : "Part " + p.part;
    }
    ...
    function applyTheme(t) {
      ...
      try {
        localStorage.setItem("pmn-theme", t);
      } catch (e) {
      }
      var tog = g("theme-tog");
    ...
    function initTheme() {
      try {
        applyTheme(localStorage.getItem("pmn-theme") || "dark");
      } catch (e) {
        applyTheme("dark");
      }
    }
    ...
    function isManualLayout() {
      try {
        return localStorage.getItem("pmn-layout");
      } catch (e) {
        return null;
      }
    }
    function applyLayout(mode) {
      ...
      try {
        if (mode) localStorage.setItem("pmn-layout", mode);
        else localStorage.removeItem("pmn-layout");
      } catch (e) {
      }
      updateLayoutTog();
      if (mode === "desktop" && !sbOpen) {
        var sb = g("sidebar");
    ...
    function setReaderScale(scale) {
      ...
      try {
        localStorage.setItem("pmn-reader-scale", String(scale));
      } catch (e) {
      }
    }
    ...
    function setSidebarScale(scale) {
      ...
      try {
        localStorage.setItem("pmn-sidebar-scale", String(scale));
      } catch (e) {
      }
    }
    ...
    function setReaderMeasure(measure) {
      ...
      try {
        localStorage.setItem("pmn-reader-measure", measure);
      } catch (e) {
      }
    }
    function updateReaderAgentStatus() {
      var p = PARTS2[curP], s = p.subs[curS];
      ...
      var retrieved = pack.sections.map(function(item) {
        return item.id;
      }).join(", ");
      var terms = pack.glossary.map(function(item) {
        return item.term;
      }).join(", ");
    ...
    function injectReadingPaths() {
      ...
      wrap.innerHTML = '<div class="reading-paths-hdr"><h2>Reading Paths</h2><p>Not every reader needs to start the same way. These entry paths give faster on-ramps into PMN depending on whether you want foundations, power analysis, formula compression, or applied cases.</p></div><div class="reading-paths-meta"><div class="reading-stat"><strong>Entry logic</strong><span>Choose by task, not by obligation.</span></div><div class="reading-stat"><strong>Fastest route</strong><span>15.15 for compression, then backfill.</span></div><div class="reading-stat"><strong>Best for first pass</strong><span>Start with foundations, not slogans.</span></div></div><div class="reading-paths-grid"><article class="path-card" data-ghost="01"><span class="path-kicker">Path 01</span><h3>Foundation First</h3><p>Start with epistemology, ontology, and the biological floor before touching doctrine or applied cases.</p><button class="path-btn" type="button" data-open-part="I">Open Part I</button></article><article class="path-card" data-ghost="02"><span class="path-kicker">Path 02</span><h3>Power and Institutions</h3><p>Jump straight into how power, legitimacy, and institutional capture shape the arrangement beneath the narrative.</p><button class="path-btn" type="button" data-open-part="VI">Open Part VI</button></article><article class="path-card" data-ghost="03"><span class="path-kicker">Path 03</span><h3>Compressed Core</h3><p>Use the short-form PMN core when you need the framework fast before going back for the full architecture.</p><button class="path-btn" type="button" data-open-id="15.15">Open 15.15</button></article><article class="path-card" data-ghost="04"><span class="path-kicker">Path 04</span><h3>Cases and the Individual</h3><p>Move from abstract structure into historical cases and the practical demands PMN places on a person who holds it.</p><button class="path-btn" type="button" data-open-part="XVII">Open Part XVII</button></article></div>';
    ...
    function upgradeHomeAi() {
      ...
      inner.innerHTML = '<div class="home-ai-hdr">Dialectical Synthesis Terminal (AI-Powered)</div><p class="home-ai-desc">Ask a PMN question here, choose your target AI, then let the site retrieve relevant PMN passages and glossary terms from the manuscript before opening ChatGPT or Gemini.</p><div class="home-ai-row"><select id="home-ai-mode" class="home-ai-select"><option value="local">Target: Local AI (In-Browser Expert)</option><option value="chatgpt">Target: ChatGPT</option><option value="gemini">Target: Gemini</option></select><input type="text" id="home-ai-input" class="home-ai-input" placeholder="e.g. Explain PMN on institutional betrayal, or where should a new reader start?" autocomplete="off"><button id="home-ai-btn" class="home-ai-btn" type="button">Ask Agent</button></div><div class="home-ai-terminal" id="home-ai-terminal">&gt;&gt; PMN LOCAL EXPERT AGENT ACTIVE\n&gt;&gt; SELECT "LOCAL AI" FOR IN-BROWSER RESPONSES.\n&gt;&gt; ASK A QUESTION TO GENERATE DEEP ANALYSIS INSTANTLY.</div><div class="home-ai-actions"><button id="home-ai-copy" class="pmn-agent-btn" type="button">Copy prompt only</button></div><p class="home-ai-note">The local AI expert generates responses entirely in-browser, referencing the full manuscript offline.</p>';
    }
    function pip2() {
      var tot = 0;
      for (var i = 0; i < PARTS2.length; i++) {
        var ps = PARTS2[i].subs;
        for (var j = 0; j < ps.length; j++) {
          tot++;
          if (ps[j].subs) for (var k = 0; k < ps[j].subs.length; k++) {
            tot++;
            if (ps[j].subs[k].subs) tot += ps[j].subs[k].subs.length;
          }
        }
      }
      var done = Object.keys(readMap2).length;
      var pct = tot ? Math.round(done / tot * 100) : 0;
      g("pip-fill").style.width = pct + "%";
      if (g("pip-num")) g("pip-num").textContent = done + "/" + tot;
      g("st-p").textContent = PARTS2.length;
      g("st-s").textContent = tot;
      g("st-r").textContent = pct + "%";
      if (g("toc-stat-parts")) g("toc-stat-parts").textContent = PARTS2.length;
      if (g("toc-stat-sections")) g("toc-stat-sections").textContent = tot;
      if (g("toc-stat-read")) g("toc-stat-read").textContent = pct + "%";
      updateHomeSnapshot(PARTS2.length, tot, pct);
    }
    function nav(v) {
    ...
    function buildTOC2() {
      ...
      for (var pi = 0; pi < PARTS2.length; pi++) {
        ...
        for (var si = 0; si < p.subs.length; si++) {
          ...
          if (r) done++;
          subs += '<button class="toc-sub' + (intro ? " intro" : "") + '" data-pi="' + pi + '" data-si="' + si + '">' + (intro ? "" : '<span class="toc-sid' + (r ? " r" : "") + '">' + esc2(displaySectionId(s.id)) + "</span>") + '<span class="toc-sname' + (r ? " r" : "") + '">' + esc2(s.title) + "</span>" + (r ? '<span class="toc-chk">&#10003;</span>' : "") + "</button>";
        }
        var pct = total ? Math.round(done / total * 100) : 0;
        var progBar = total > 1 ? '<div class="toc-prog"><div class="toc-prog-bar"><div class="toc-prog-fill" style="width:' + pct + '%"></div></div><span class="toc-prog-lbl">' + done + "/" + total + "</span></div>" : "";
        html += '<div class="toc-part"><button class="toc-ph" data-pi="' + pi + '" data-si="0">' + (lbl ? '<span class="toc-pnum">' + esc2(lbl) + "</span>" : "") + '<span class="toc-pname">' + esc2(p.title) + '</span></button><div class="toc-subs">' + subs + "</div>" + progBar + "</div>";
      }
      g("toc-grid").innerHTML = html;
      var btns = g("toc-grid").querySelectorAll("[data-pi]");
      for (var i = 0; i < btns.length; i++) {
        btns[i].addEventListener("click", /* @__PURE__ */ function(b) {
          return function() {
            openSec(+b.getAttribute("data-pi"), +b.getAttribute("data-si"));
          };
        }(btns[i]));
      }
    }
    function buildSB() {
      ...
      for (var pi = 0; pi < PARTS2.length; pi++) {
        var p = PARTS2[pi];
        html += '<div class="sb-plbl' + (pi === curP ? " on" : "") + '">' + esc2(pshort(p)) + "</div>";
        for (var si = 0; si < p.subs.length; si++) {
          var s = p.subs[si], on = pi === curP && si === curS, r = isR(pi, si), intro = s.is_intro;
          html += '<button class="sb-item' + (on ? " on" : "") + (intro ? " intro-sec" : "") + '" data-pi="' + pi + '" data-si="' + si + '"><span class="sb-iid' + (on ? " on" : r ? " r" : "") + '">' + esc2(displaySectionId(s.id)) + '</span><span class="sb-ilbl' + (on ? " on" : "") + '">' + esc2(s.title) + "</span></button>";
        }
      }
    ...
    function getSectionMeta(pi, si) {
      if (typeof pi !== "number" || typeof si !== "number" || !PARTS2[pi] || !PARTS2[pi].subs[si]) return null;
      var s = PARTS2[pi].subs[si];
      return { kind: "section", pi, si, id: s.id, title: s.title };
    }
    ...
    function openPartStart(part, opts) {
      for (var pi = 0; pi < PARTS2.length; pi++) {
        if (PARTS2[pi].part === part) {
          if (PARTS2[pi].subs && PARTS2[pi].subs.length > 0) {
    ...
    function linkXrefs(container) {
      var pat = XREF_IDS.map(function(id) {
        return id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      }).join("|");
    ...
    function openSec(pi, si, nohash) {
      ...
      renderSec();
      nav("reader");
      if (typeof window._afterOpenSec === "function") window._afterOpenSec(pi, si);
      if (!nohash) {
        try {
          history.pushState(null, "", "#" + PARTS2[pi].subs[si].id);
        } catch (e) {
        }
      }
      if (window.innerWidth <= 680) {
      ...
      updateJumpBack();
      setTimeout(function() {
        var a = g("sb-list").querySelector(".sb-item.on");
        if (a) a.scrollIntoView({ block: "nearest", behavior: "smooth" });
      }, 60);
    }
    function openById(sid) {
      return jumpToSectionById(sid, { nohash: true });
    }
    function clean(t) {
    ...
    function addHist(pi, si) {
      var s = PARTS2[pi].subs[si], key = pi + "-" + si;
      sessHist = sessHist.filter(function(h) {
        return h.pi + "-" + h.si !== key;
      });
      sessHist.unshift({ pi, si, id: s.id, title: s.title });
      if (sessHist.length > 12) sessHist.pop();
      renderHist();
    }
    function renderHist() {
      var wrap = g("sb-hist"), list = g("sb-hist-list");
      if (!wrap || !list) return;
      if (sessHist.length < 2) {
        wrap.style.display = "none";
        return;
      }
      wrap.style.display = "";
      ...
      for (var i = 1; i < sessHist.length; i++) {
        var e = sessHist[i];
        h += '<div class="sb-hist-item" data-pi="' + e.pi + '" data-si="' + e.si + '"><span class="sb-hist-sid">' + esc2(displaySectionId(e.id)) + '</span><span class="sb-hist-title">' + esc2(e.title.length > 32 ? e.title.slice(0, 30) + "\u2026" : e.title) + "</span></div>";
      }
      list.innerHTML = h;
    }
    ...
    function renderSuggestions() {
      ...
      var done = Object.keys(readMap2).length, pct = tot ? done / tot : 0;
      ...
      for (var i = 0; i < ents.length; i++) {
        var sid = ents[i][0], why = ents[i][1], info = LOOK[sid];
        if (!info) continue;
        h += '<button class="suggest-btn" data-pi="' + info.pi + '" data-si="' + info.si + '"><span class="suggest-sid">' + esc2(sid) + '</span><span class="suggest-title">' + esc2(PARTS2[info.pi].subs[info.si].title) + '</span><span class="suggest-why">' + esc2(why) + "</span></button>";
        shown++;
      }
      if (shown > 0) {
        wrap.innerHTML = h;
    ...
    function renderCitedIn(sid) {
      ...
      for (var i = 0; i < refs.length; i++) {
        ...
        if (!info) continue;
        h += '<button class="cited-btn" data-pi="' + info.pi + '" data-si="' + info.si + '"><span class="cited-sid">' + esc2(r) + "</span>" + esc2(info.title.length > 40 ? info.title.slice(0, 38) + "\u2026" : info.title) + "</button>";
      }
      h += "</div>";
      el2.innerHTML = h;
    }
    function annotKey(pi, si) {
      return "pmn-an-" + PARTS2[pi].subs[si].id;
    }
    function loadAnnot(pi, si) {
      var ta = g("annot-ta");
      if (!ta) return;
      try {
        ta.value = localStorage.getItem(annotKey(pi, si)) || "";
      } catch (e) {
        ta.value = "";
      }
    }
    ...
    function loadHomeNotes() {
      var ta = g("home-notes-ta");
      if (!ta) return;
      try {
        ta.value = localStorage.getItem("pmn-home-notes") || "";
      } catch (e) {
        ta.value = "";
      }
    }
    ...
    function clearHomeNotes() {
      ...
      ta.value = "";
      try {
        localStorage.removeItem("pmn-home-notes");
      } catch (e) {
      }
    ...
    function copyHomeNotes() {
      ...
      if (!text) {
        showHomeNotesStatus("Nothing to copy");
        return;
      }
    ...
    function openAnnotSummary() {
      var entries = [];
      for (var pi = 0; pi < PARTS2.length; pi++) {
        for (var si = 0; si < PARTS2[pi].subs.length; si++) {
          var s = PARTS2[pi].subs[si], note = "";
          try {
            note = localStorage.getItem("pmn-an-" + s.id) || "";
          } catch (e) {
          }
      ...
      if (!entries.length) {
        ...
        body.innerHTML = entries.map(function(e) {
          return '<div style="margin-bottom:2rem;padding-bottom:2rem;border-bottom:1px solid var(--rule)"><div style="display:flex;gap:.6rem;align-items:baseline;margin-bottom:.5rem"><span style="font-family:Source Code Pro,monospace;font-size:.7rem;color:var(--acc)">' + esc2(e.id) + '</span><span style="font-family:Lora,serif;font-size:.9rem;color:var(--mute)">' + esc2(e.title) + '</span></div><pre style="font-family:Lora,serif;font-size:.95rem;line-height:1.75;white-space:pre-wrap;color:var(--ink2);margin:0">' + esc2(e.note) + "</pre></div>";
        }).join("");
      }
    ...
    function renderRelated(sid) {
      ...
      for (var i = 0; i < refs.length; i++) {
        ...
        h += '<button class="rel-btn" data-pi="' + info.pi + '" data-si="' + info.si + '"><span class="rel-sid">' + esc2(r) + '</span><span class="rel-stitle">' + esc2(info.title) + '</span><span class="rel-pt">' + esc2(ptlbl) + "</span></button>";
      }
      h += "</div>";
      el2.innerHTML = h;
    }
    function renderSec() {
      ...
      g("sec-ttl").innerHTML = (isSpecialSec ? "" : '<span class="sec-id">' + esc2(s.id) + "</span>") + esc2(s.title) + shareBtn;
      g("sec-ttl").className = isSpecialSec ? "sec-ttl intro-ttl" : "sec-ttl";
      var html = s.html || (s.text ? "<p>" + esc2(s.text) + "</p>" : "");
      ...
      col.classList.remove("fi");
      void col.offsetWidth;
      col.classList.add("fi");
      var first = curP === 0 && curS === 0, last = curP === PARTS2.length - 1 && curS === PARTS2[curP].subs.length - 1;
      ...
      if (metaPanel) {
        ...
        if (metaPanel.parentElement) {
          if (!hasRefs) {
            metaPanel.parentElement.classList.add("single-panel");
          } else {
            metaPanel.parentElement.classList.remove("single-panel");
          }
        }
      }
      ...
      var fl = g("feedback-link");
      if (fl) fl.setAttribute("href", "mailto:novadharma.pm@gmail.com?subject=" + subj + "&body=" + body);
      var tweet = encodeURIComponent("Re: PMN Reader - " + s.id + " " + s.title + " @nova_dharma_pm");
      var tl = g("twitter-link");
      if (tl) tl.setAttribute("href", "https://twitter.com/intent/tweet?text=" + tweet);
    }
    function nextSec(pi, si) {
      si++;
      if (si >= PARTS2[pi].subs.length) {
        pi++;
        si = 0;
      }
      if (pi >= PARTS2.length) return;
      if (PARTS2[pi].subs[si] && PARTS2[pi].subs[si].is_intro) {
        si++;
        if (si >= PARTS2[pi].subs.length) {
          pi++;
          si = 0;
      ...
      if (pi < PARTS2.length) openSec(pi, si);
    }
    ...
    function updateMobNav() {
      ...
      if (nn) nn.disabled = curP === PARTS2.length - 1 && curS === PARTS2[curP].subs.length - 1;
    }
    var _st = null;
    function onSI(v) {
    ...
    function renderGLLegacyUnused() {
      ...
      for (var i = 0; i < grps.length; i++) {
        ...
        h += '<div class="gl-grp-lbl">' + esc2(grp) + '</div><div class="gl-grid">';
        for (var j = 0; j < terms.length; j++) {
          var t = terms[j], d = GL[t] || "";
          h += '<button class="gl-card" data-term="' + esc2(t) + '" title="Search sections mentioning this term"><p class="gl-term">' + esc2(t) + '</p><p class="gl-def">' + esc2(d) + "</p></button>";
        }
        h += "</div>";
      }
    ...
    function renderGL() {
      ...
      for (var i = 0; i < grps.length; i++) {
        ...
        for (var j = 0; j < terms.length; j++) {
          ...
          h += '<button class="gl-card" data-term="' + esc2(t) + '" title="Search sections mentioning this term"><p class="gl-term">' + esc2(t) + '</p><p class="gl-def' + (empty ? " is-empty" : "") + '">' + esc2(d) + "</p></button>";
        }
        h += "</div>";
      }
      g("sv-body").innerHTML = h;
    ...
    function populatePartFilter() {
      ...
      for (var pi = 0; pi < PARTS2.length; pi++) {
        ...
        o.value = String(pi);
        o.textContent = pshort(PARTS2[pi]);
        sel.appendChild(o);
      }
      sel.addEventListener("change", function() {
        doSrch(g("srch-in").value);
      });
    }
    function doSrch(q) {
      hideContentsPanel();
      ...
      if (/^[0-9]+\.[0-9]+[a-z]?$/.test(ql) || /^a\.[0-9]+$/i.test(ql)) {
        var info = LOOK[resolveSectionId(ql)];
        if (info) {
          jumpToSection(info.pi, info.si, { origin: getJumpOrigin() });
          clrS();
          return;
        }
      }
      ...
      for (var pi = 0; pi < PARTS2.length; pi++) {
        ...
        for (var si = 0; si < p.subs.length; si++) {
          ...
          if (ql && (s.title.toLowerCase().indexOf(ql) >= 0 || stxt.toLowerCase().indexOf(ql) >= 0)) {
            ...
            var words = ql.split(/\s+/).filter(function(w2) {
              return w2.length > 2;
            });
            if (words.length >= 2) {
          ...
          if (matched) {
            res.push({ pi, si, p, s, snip });
            addedSections[key] = true;
          }
        }
      }
      function hl(t) {
        if (!q || q.length < 2) return esc2(t);
        return esc2(t).replace(new RegExp("(" + q.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + ")", "gi"), "<mark>$1</mark>");
      }
      g("sv-hdr").textContent = res.length + " result" + (res.length !== 1 ? "s" : "") + " for \u201C" + q + "\u201D";
      var shown = res.slice(0, 40), h = "";
      for (var i = 0; i < shown.length; i++) {
        var r = shown[i];
        h += '<button class="res' + (r.isGlossaryMatch ? " is-glossary-match" : "") + '" data-pi="' + r.pi + '" data-si="' + r.si + '"><p class="res-loc">' + esc2(pshort(r.p)) + " \xB7 " + esc2(r.s.id) + '</p><p class="res-title">' + hl(r.s.title) + "</p>" + (r.snip ? '<p class="res-snip">' + (r.isGlossaryMatch ? r.snip : hl(r.snip)) + "</p>" : "") + "</button>";
      }
    ...
    function handleSel(e) {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (txt === keys[i] || txt.indexOf(keys[i]) >= 0) {
          key = keys[i];
          break;
        }
      }
    ...
    document.addEventListener("keydown", function(e) {
      ...
      if (e.key === "Escape") {
        if (g("kbd-modal") && g("kbd-modal").style.display === "block") {
          closeKbd();
          return;
        }
        if (g("notes-modal") && g("notes-modal").style.display === "block") {
          closeNotesModal();
          return;
        }
        if (g("srch-view") && g("srch-view").classList.contains("on")) {
          clrS();
          return;
        }
      }
    ...
    g("sv-body").addEventListener("click", function(e) {
      ...
      var btn = e.target.closest("[data-pi]");
      if (!btn) return;
    ...
    g("hdr-logo").addEventListener("click", function() {
      nav("home");
    });
    g("hb-home").addEventListener("click", function() {
      nav("srch");
      showContentsPanel();
    });
    g("hb-gl") && g("hb-gl").addEventListener("click", showGL);
    g("mob-prev") && g("mob-prev").addEventListener("click", function() {
      navP();
    });
    g("mob-next") && g("mob-next").addEventListener("click", function() {
      navN();
    });
    g("mob-mid") && g("mob-mid").addEventListener("click", function() {
      nav("srch");
      showContentsPanel();
    });
    ...
    g("srch-in").addEventListener("input", function() {
      onSI(this.value);
    });
    g("srch-in").addEventListener("focus", onSF);
    g("cta-begin").addEventListener("click", function() {
      nav("srch");
      showContentsPanel();
    });
    g("cta-gl").addEventListener("click", showGL);
    g("sb-tog").addEventListener("click", toggleSB);
    g("bc-home").addEventListener("click", function() {
      nav("srch");
      showContentsPanel();
    });
    ...
    populatePartFilter();
    renderSuggestions();
    g("footer-home").addEventListener("click", function() {
      nav("home");
    });
    g("home-bottom-glossary") && g("home-bottom-glossary").addEventListener("click", showGL);
    g("home-bottom-open-contents") && g("home-bottom-open-contents").addEventListener("click", function() {
      nav("srch");
      showContentsPanel();
    });
    ...
    g("font-sm") && g("font-sm").addEventListener("click", function() {
      setReaderScale(0.92);
    });
    g("font-md") && g("font-md").addEventListener("click", function() {
      setReaderScale(1);
    });
    g("font-lg") && g("font-lg").addEventListener("click", function() {
      setReaderScale(1.08);
    });
    g("contents-sm") && g("contents-sm").addEventListener("click", function() {
      setContentsScale(0.94);
    });
    g("contents-md") && g("contents-md").addEventListener("click", function() {
      setContentsScale(1);
    });
    g("contents-lg") && g("contents-lg").addEventListener("click", function() {
      setContentsScale(1.1);
    });
    g("sidebar-sm") && g("sidebar-sm").addEventListener("click", function() {
      setSidebarScale(0.92);
    });
    g("sidebar-md") && g("sidebar-md").addEventListener("click", function() {
      setSidebarScale(1);
    });
    g("sidebar-lg") && g("sidebar-lg").addEventListener("click", function() {
      setSidebarScale(1.1);
    });
    g("measure-narrow") && g("measure-narrow").addEventListener("click", function() {
      setReaderMeasure("62ch");
    });
    g("measure-base") && g("measure-base").addEventListener("click", function() {
      setReaderMeasure("68ch");
    });
    g("measure-wide") && g("measure-wide").addEventListener("click", function() {
      setReaderMeasure("76ch");
    });
    ...
    var _pos = loadPos();
    if (_pos && _pos[0] < PARTS2.length && _pos[1] < PARTS2[_pos[0]].subs.length) {
      var _rs = PARTS2[_pos[0]].subs[_pos[1]];
      var _rb = g("cta-resume");
      _rb.textContent = "Resume: " + _rs.id + " \u2192";
      _rb.style.display = "";
      _rb.addEventListener("click", function() {
        jumpToSection(_pos[0], _pos[1]);
      });
    }
    ...
    g("reader-open-contents") && g("reader-open-contents").addEventListener("click", function() {
      nav("srch");
      showContentsPanel();
    });
    ...
    g("sidebar").addEventListener("click", function(e) {
      ...
      if (h) {
        jumpToSection(+h.getAttribute("data-pi"), +h.getAttribute("data-si"), { origin: getJumpOrigin() });
      }
    });
    g("reader-col") && g("reader-col").addEventListener("click", function(e) {
      var cb = e.target.closest(".cited-btn");
      if (cb) {
        jumpToSection(+cb.getAttribute("data-pi"), +cb.getAttribute("data-si"), { origin: getJumpOrigin() });
      }
    });
    g("suggestions") && g("suggestions").addEventListener("click", function(e) {
      var b = e.target.closest(".suggest-btn");
      if (b) {
        jumpToSection(+b.getAttribute("data-pi"), +b.getAttribute("data-si"), { origin: getJumpOrigin() });
      }
    });
    ...
    (function() {
      ...
      function next() {
        goTo((cur + 1) % QUOTES.length, false);
      }
      function startTimer() {
        timer = setInterval(next, 18e3);
      }
      showQuote(cur);
      startTimer();
    })();
  ...
  (function() {
    var PARTS2 = function() {
      try {
        return JSON.parse(document.getElementById("d-parts").textContent);
      } catch (e) {
        return [];
      }
    }();
    ...
    if (!sb || !ac || !PARTS2.length) return;
    var SPECIAL_PARTS = ["Preface", "Coda", "Intellectual Debts", "Bibliography"];
    var display = PARTS2.filter(function(p) {
      return SPECIAL_PARTS.indexOf(p.part) < 0;
    });
    ...
    display.forEach(function(part, i) {
      ...
      panelsHtml += '<div class="anatomy-panel' + (i === 0 ? " on" : "") + '" id="' + id + '"><div class="ap-badge">' + partLabel.toUpperCase() + "</div><h3>" + part.title + "</h3>" + (descText ? '<p style="font-size:.9rem;color:var(--ink2);line-height:1.65;margin-bottom:1.2rem">' + descText + "</p>" : "") + '<div style="margin-top:.8rem">' + secList + "</div></div>";
    });
    sb.innerHTML = tabsHtml;
    ac.innerHTML = panelsHtml;
    sb.querySelectorAll(".anatomy-tab").forEach(function(btn) {
      btn.addEventListener("click", function() {
        sb.querySelectorAll(".anatomy-tab").forEach(function(b) {
          b.classList.remove("on");
        });
        ac.querySelectorAll(".anatomy-panel").forEach(function(p) {
...
.pmn-chat-body.thinking{color:var(--mute);font-style:italic;animation:blink 1s infinite}
  ...
  (function() {
    function callClaude(messages, sysPrompt, onDone, onError) {
      ...
      function stream() {
        if (index < words.length) {
          ...
          var homeLog = document.getElementById("home-chat-log");
          if (homeLog) {
            window.linkXrefs(homeLog);
            homeLog.scrollTop = homeLog.scrollHeight;
          }
          var pmnLog = document.getElementById("pmn-chat-log");
          if (pmnLog) {
            window.linkXrefs(pmnLog);
    ...
    var PARTS2, LOOK, REL, GL;
    function ensureData2() {
      if (!PARTS2) {
        try {
          PARTS2 = JSON.parse(document.getElementById("d-parts").textContent);
        } catch (e) {
          PARTS2 = [];
        }
        try {
          LOOK = JSON.parse(document.getElementById("d-look").textContent);
        } catch (e) {
          LOOK = {};
        }
        try {
          REL = JSON.parse(document.getElementById("d-rel").textContent);
        } catch (e) {
          REL = {};
        }
        try {
          GL = JSON.parse(document.getElementById("d-gl").textContent);
        } catch (e) {
          GL = {};
        }
      }
    ...
    function truncate(str, n) {
      return stripHtml(str).slice(0, n);
    }
    function fullText(s, maxChars) {
      return stripHtml(s.html || s.text || "").slice(0, maxChars || 3e3);
    }
    function buildContextPack2(query, opts) {
      ensureData2();
      opts = opts || {};
      var q = (query || "").toLowerCase();
      var scored = [];
      for (var pi = 0; pi < PARTS2.length; pi++) {
        ...
        for (var si = 0; si < p.subs.length; si++) {
          ...
          q.split(/\s+/).filter(function(w) {
            return w.length > 3;
          }).forEach(function(w) {
            if (blob.indexOf(w) >= 0) score++;
          });
          ...
          if (score > 0) scored.push({ pi, si, s, p, score });
        }
      }
      ...
      var sections = scored.slice(0, maxSec).map(function(item) {
        return {
          ...
          fullText: fullText(item.s, opts.fullTextChars || 2e3)
        };
      });
      var glossary = [];
      if (q.length > 2) {
    ...
    function formatPackFull(pack, activeSec) {
      ...
      var others = activeSec ? pack.sections.filter(function(s) {
        return s.id !== activeSec.id;
      }) : pack.sections;
      if (others.length) {
    ...
    (function() {
      ...
      function switchTab(mode) {
        ...
        Object.keys(panels).forEach(function(k) {
          if (panels[k]) panels[k].style.display = k === mode ? "" : "none";
        });
      }
      tabs.forEach(function(btn2) {
        btn2.addEventListener("click", function() {
          switchTab(btn2.getAttribute("data-mode"));
        });
      });
      switchTab("chatgpt");
      ...
      function ask() {
        ...
        var pack = buildContextPack2(q, { maxSections: 5, maxGlossary: 5, fallbackIds: ["how-to-read-this-document", "3.0", "6.1", "10.1", "15.15", "17.0"] });
        var userMsg = buildPrompt("agent", q, pack, null);
        var msgs = history2.slice(0, -1).concat([{ role: "user", content: userMsg }]);
        callClaude(msgs, SYS, function(text) {
          thinkEl.className = "hcl-body";
          thinkEl.textContent = text;
          history2.push({ role: "assistant", content: text });
          if (btn) {
            btn.disabled = true;
            btn.textContent = "Backend required";
          }
          chatLog.scrollTop = chatLog.scrollHeight;
        }, function(err) {
          thinkEl.className = "hcl-body";
          thinkEl.textContent = "Error: " + err;
          if (btn) {
            btn.disabled = true;
            btn.textContent = "Backend required";
          }
      ...
      if (exportBtn) exportBtn.addEventListener("click", function() {
        ...
        var txt = history2.map(function(m) {
          return (m.role === "user" ? "YOU: " : "PMN AGENT: ") + m.content;
        }).join("\n\n---\n\n");
        navigator.clipboard.writeText(txt).then(function() {
          var o = exportBtn.textContent;
          exportBtn.textContent = "Copied!";
          setTimeout(function() {
            exportBtn.textContent = o;
          }, 1800);
        });
      });
      function setupRedirect(qId, btnId, cpyId, statId, modeId, url) {
        ...
        function go(openWin) {
          ...
          var pack = buildContextPack2(q, { maxSections: 5, maxGlossary: 6, fullTextChars: 2500, fallbackIds: ["how-to-read-this-document", "3.0", "6.1", "10.1", "15.15", "17.0"] });
          ...
          navigator.clipboard.writeText(full).then(function() {
            ...
            if (stat) stat.textContent = openWin ? "Prompt copied (" + pack.sections.length + " sections, ~" + kb + "KB of manuscript text). Paste into " + url.split("/")[2] + "." : "Copied (" + pack.sections.length + " sections, ~" + kb + "KB). Paste into any AI.";
            if (qEl) qEl.value = "";
          }).catch(function() {
            if (stat) stat.textContent = "Clipboard blocked. Try copy-only.";
          });
        ...
        if (cpyBtn) cpyBtn.addEventListener("click", function() {
          go(false);
        });
        if (qEl) qEl.addEventListener("keypress", function(e) {
          if (e.key === "Enter") {
            e.preventDefault();
            go(true);
          }
    ...
    (function() {
      ...
      function switchTab(mode) {
        tabs.forEach(function(b) {
          b.classList.toggle("active", b.getAttribute("data-rmode") === mode);
        });
        Object.keys(panels).forEach(function(k) {
          if (panels[k]) panels[k].style.display = k === mode ? "" : "none";
        });
      ...
      tabs.forEach(function(btn) {
        btn.addEventListener("click", function() {
          switchTab(btn.getAttribute("data-rmode"));
        });
      });
      switchTab("chatgpt");
      ...
      function getActiveSec() {
        if (window._pmnParts && window._pmnCurP !== void 0) {
      ...
      function ask() {
        ...
        var pack = buildContextPack2(q, { activeId: activeSec && activeSec.id, relatedIds: relIds, maxSections: 4, maxGlossary: 4 });
        var userMsg = buildPrompt("agent", q, pack, activeSec);
        var msgs = rHistory.slice(0, -1).concat([{ role: "user", content: userMsg }]);
        callClaude(msgs, SYS, function(text) {
          thinkEl.className = "pmn-chat-body";
          thinkEl.textContent = text;
          rHistory.push({ role: "assistant", content: text });
          if (sendBtn) {
            sendBtn.disabled = true;
            sendBtn.textContent = "Backend required";
          }
          chatLog.scrollTop = chatLog.scrollHeight;
        }, function(err) {
          thinkEl.className = "pmn-chat-body";
      ...
      if (chatIn) chatIn.addEventListener("keypress", function(e) {
        if (e.key === "Enter") {
          e.preventDefault();
          ask();
        }
        ...
        function go(openWin) {
          ...
          if (activeSec) {
            activeSecFull = {
              ...
              fullText: fullText(activeSec, 4e3)
              // Full active section up to 4000 chars
            };
          }
          var pack = buildContextPack2(q, {
          ...
          navigator.clipboard.writeText(full).then(function() {
            var chars = full.length;
            var kb = Math.round(chars / 1024);
            if (stat) stat.textContent = openWin ? "Prompt copied (~" + kb + "KB with full section text). Paste into " + url.split("/")[2] + "." : "Copied (~" + kb + "KB of manuscript text). Paste into any AI.";
            if (qEl) qEl.value = "";
          }).catch(function() {
            if (stat) stat.textContent = "Clipboard blocked.";
          });
      ...
      window._pmnClearReaderChat = function() {
        rHistory = [];
    ...
    (function() {
      ...
      function loadHL() {
        try {
          highlights = JSON.parse(localStorage.getItem(HL_KEY) || "{}");
        } catch (e) {
          highlights = {};
        }
      }
      ...
      document.addEventListener("mouseup", function(e) {
        ...
        setTimeout(function() {
          ...
          curSel = { text, range: range.cloneRange() };
          editHlId = null;
      ...
      function wrapSpan(range, hlId, color, note) {
        try {
          ...
          span.title = note ? "\u{1F4DD} " + note : "Click to annotate";
      ...
      function attachSpanClick(span, hlId) {
        span.addEventListener("click", function(e) {
          ...
          curSel = null;
          var secId = getSecId();
          var hl = (highlights[secId] || []).find(function(h) {
            return h.id === hlId;
          });
      ...
      function applyHL(color, note) {
        ...
        highlights[secId].push({ id: hlId, text: curSel.text, color, note: note || "" });
        saveHL();
      ...
      document.getElementById("hl-save-note-btn").addEventListener("click", function(e) {
        ...
        if (editHlId && secId) {
          ...
          var sp = document.querySelector('[data-hl-id="' + editHlId + '"]');
          if (sp) sp.title = note ? "\u{1F4DD} " + note : "Click to annotate";
          hideToolbar();
        } else if (curSel) {
          applyHL("red", note);
        }
      ...
      document.getElementById("hl-remove-btn").addEventListener("click", function(e) {
        ...
        var secId = getSecId();
        if (editHlId && secId) {
          highlights[secId] = (highlights[secId] || []).filter(function(h) {
            return h.id !== editHlId;
          });
          saveHL();
          var sp = document.querySelector('[data-hl-id="' + editHlId + '"]');
      ...
      function updateBadge(secId) {
        var badge2 = document.getElementById("hl-count-badge");
        if (!badge2) return;
        var n = (highlights[secId] || []).length;
        badge2.textContent = n ? n + " highlight" + (n > 1 ? "s" : "") : "Highlights";
      }
      function showHlModal() {
        ...
        if (!sHls.length) {
          ...
          body.innerHTML = sHls.map(function(h) {
            ...
            return '<div style="margin-bottom:1.4rem;padding-bottom:1.4rem;border-bottom:1px solid var(--rule)">' + dot + '<span style="font-family:Lora,serif;font-size:.9rem;color:var(--ink2);font-style:italic">&ldquo;' + h.text.slice(0, 120) + (h.text.length > 120 ? "\u2026" : "") + "&rdquo;</span>" + (h.note ? '<div style="font-family:Source Code Pro,monospace;font-size:.75rem;color:var(--mute);margin-top:.3rem;margin-left:1.2rem">\u{1F4DD} ' + h.note + "</div>" : "") + "</div>";
          }).join("");
        }
        modal.style.display = "block";
        document.body.style.overflow = "hidden";
      ...
      function renderHL(secId) {
        ...
        sHls.forEach(function(hl) {
          var esc2 = hl.text.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
          var cls = "pmn-hl" + (hl.color && hl.color !== "red" ? " hl-" + hl.color : "");
          var titleAttr = hl.note ? 'title="\u{1F4DD} ' + hl.note.replace(/"/g, "&quot;") + '"' : "";
          html = html.replace(new RegExp(`(?<!['"=>])(` + esc2 + ")(?![^<]*>)"), '<span class="' + cls + '" data-hl-id="' + hl.id + '" ' + titleAttr + ">$1</span>");
        });
      ...
      window._pmnRenderHL = function(secId) {
        renderHL(secId);
        updateBadge(secId);
      };
    })();
    (function() {
      ...
      srchIn.addEventListener("input", function() {
        ...
        if (!semanticOn || q.length < 4) {
          semBar.classList.remove("visible");
          return;
        }
      ...
      function doSemantic(q) {
        ...
        for (var pi = 0; pi < PARTS2.length; pi++) {
          ...
          for (var si = 0; si < p.subs.length; si++) {
            ...
            var score = 0;
            q.toLowerCase().split(/\s+/).filter(function(w) {
              return w.length > 2;
            }).forEach(function(w) {
              var i = blob.indexOf(w);
              while (i >= 0 && score < 30) {
                score++;
                i = blob.indexOf(w, i + 1);
              }
            });
            Object.keys(GL || {}).forEach(function(term) {
        ...
        if (!top.length) {
          semBar.textContent = 'No results for "' + q + '"';
          return;
        }
    ...
    window.buildContextPack = buildContextPack2;
    window.ensureData = ensureData2;
    window.stripHtml = stripHtml;
    setTimeout(function() {
      if (window.ensureData) window.ensureData();
      window._pmnParts = window.PARTS || PARTS2;
  ...
  (function() {
    ...
    list.addEventListener("click", function(e) {
      ...
      list.querySelectorAll(".thesis-item").forEach(function(i) {
  ...
  (function() {
    ...
    Particle.prototype.update = function() {
      this.x += this.vx + Math.sin(this.y * 8e-3 + this.sizeSeed) * 0.22;
      this.y += this.vy + Math.cos(this.x * 8e-3 + this.sizeSeed) * 0.22;
      if (this.x > w) {
        this.x = w;
    ...
    Particle.prototype.draw = function(index) {
      ...
      var currentSize = this.size + Math.sin(Date.now() * 16e-4 + this.sizeSeed) * 0.35;
    ...
    function init() {
      particles = [];
      var n = Math.min(Math.floor(w * h / 6e3), 200);
      n = Math.max(n, 80);
    ...
    function injectGlossaryTooltips() {
      ...
      let glData = null;
      try {
        glData = JSON.parse(glScript.textContent);
      } catch (e) {
        return;
      }
      if (!glData) return;
      ...
      prose.querySelectorAll("p").forEach((p) => {
        ...
        while (walker.nextNode()) {
          ...
          let parent = node.parentElement, skip = false;
          while (parent && parent !== p) {
            if (parent.tagName === "A" || parent.classList.contains("glossary-term")) {
              skip = true;
              break;
            }
    ...
    (function() {
      ...
      backdrop.innerHTML = '<div class="cmd-palette-box"><div class="cmd-palette-input-wrap"><span class="cmd-palette-search-icon">&#8981;</span><input type="text" class="cmd-palette-input" placeholder="Type a command, section number, or keyword..." autocomplete="off"><span class="cmd-palette-escape-hint">ESC</span></div><div class="cmd-palette-results"></div></div>';
      ...
      function renderDefault() {
        var h2 = '<button class="cmd-palette-item" data-action="theme"><div class="cmd-palette-item-main"><span class="cmd-palette-item-title">Toggle Dark / Light Theme</span><span class="cmd-palette-item-desc">Switch between vintage bookstore and sleek dark mode</span></div><span class="cmd-palette-item-badge">Action</span></button><button class="cmd-palette-item" data-action="focus"><div class="cmd-palette-item-main"><span class="cmd-palette-item-title">Toggle Focus Mode</span><span class="cmd-palette-item-desc">Hide sidebars and read with zero distraction</span></div><span class="cmd-palette-item-badge">Action</span></button><button class="cmd-palette-item" data-action="random"><div class="cmd-palette-item-main"><span class="cmd-palette-item-title">Read a Random Section</span><span class="cmd-palette-item-desc">Browse a random emergent thesis from the manuscript</span></div><span class="cmd-palette-item-badge">Action</span></button><button class="cmd-palette-item" data-action="reset"><div class="cmd-palette-item-main"><span class="cmd-palette-item-title">Reset Reading Progress</span><span class="cmd-palette-item-desc">Clear local history and restart reading logs</span></div><span class="cmd-palette-item-badge">Action</span></button>';
      ...
      input.addEventListener("input", function() {
        ...
        (pack.glossary || []).forEach(function(item) {
          h2 += '<button class="cmd-palette-item" data-action="glossary" data-term="' + esc(item.term) + '"><div class="cmd-palette-item-main"><span class="cmd-palette-item-title">Key Term: ' + esc(item.term) + '</span><span class="cmd-palette-item-desc">' + esc(item.def) + '</span></div><span class="cmd-palette-item-badge">Glossary</span></button>';
        });
        (pack.sections || []).forEach(function(item) {
          h2 += '<button class="cmd-palette-item" data-action="section" data-id="' + esc(item.id) + '"><div class="cmd-palette-item-main"><span class="cmd-palette-item-title">' + esc(item.id) + " &mdash; " + esc(item.title) + '</span><span class="cmd-palette-item-desc">' + esc(item.part) + '</span></div><span class="cmd-palette-item-badge">Read</span></button>';
        });
        results.innerHTML = h2 || '<div style="padding: 1.5rem; text-align: center; font-family: var(--f-mono); font-size: 0.8rem; color: var(--mute);">No matching sections or terms found.</div>';
      });
      results.addEventListener("click", function(e) {
        ...
        if (action === "theme") {
          ...
          if (confirm("Are you sure you want to reset your reading progress?")) {
            readMap = {};
            try {
              localStorage.removeItem("pmn-read");
            } catch (e2) {
            }
            pip();
            buildTOC();
    ...
    (function() {
      ...
      document.addEventListener("mouseover", function(e) {
        ...
        card.innerHTML = '<span class="xref-preview-kicker">Part ' + (window.PARTS || PARTS)[info.pi].part + " &mdash; Section " + sid + '</span><h4 class="xref-preview-title">' + (window.esc ? window.esc(sec.title) : sec.title) + '</h4><p class="xref-preview-excerpt">' + (window.esc ? window.esc(excerpt) : excerpt) + '</p><span class="xref-preview-footer">&#128279; Click to jump to section</span>';
