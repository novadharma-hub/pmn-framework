(function() {
  ...
  function tokenizeSearch(txt){
    return uniqueTokens(normalizeSearchText(txt).split(' ').filter(function(token){
      return token && (token.length>2 || /\d/.test(token) || token==='pmn');
    }));
  }
  ...
  function countTokenHits(text, token){
    ...
    var match=text.match(new RegExp(escapeRegExp(token),'g'));
    return match ? match.length : 0;
  }
  function partDisplay(part){
    return SPECIAL[part.part] ? part.title : ('Part '+part.part+' - '+part.title);
  }
  var PMN_SEARCH=null;
  function ensurePmnSearchIndex(){
    if(PMN_SEARCH) return PMN_SEARCH;
    var byCategory={};
    Object.keys(GLG||{}).forEach(function(category){
      (GLG[category]||[]).forEach(function(term){
    ...
    PARTS.forEach(function(part,pi){
      part.subs.forEach(function(sub,si){
        var text=plainTextFromHtml(sub.html||sub.text||'');
        var rec={
          pi:pi,
          si:si,
          id:sub.id,
          title:sub.title,
          part:partDisplay(part),
          excerpt:truncateText(text,1200),
          titleNorm:normalizeSearchText(sub.id+' '+sub.title),
          partNorm:normalizeSearchText(partDisplay(part)),
          textNorm:normalizeSearchText(text.slice(0,7000))
        };
        PMN_SEARCH.sections.push(rec);
        PMN_SEARCH.sectionById[sub.id]=rec;
      });
    });
    Object.keys(GL||{}).forEach(function(term){
      PMN_SEARCH.glossary.push({
        term:term,
        definition:GL[term],
        category:byCategory[term]||'Glossary',
        termNorm:normalizeSearchText(term),
        defNorm:normalizeSearchText(GL[term]||'')
      });
    });
    return PMN_SEARCH;
  }
  function scoreSectionRecord(rec,tokens,phraseNorm,opts){
    var score=0, matched=[];
    if(phraseNorm){
      if(rec.titleNorm.indexOf(phraseNorm)>=0) score+=18;
      if(rec.textNorm.indexOf(phraseNorm)>=0) score+=10;
    }
    tokens.forEach(function(token){
      var local=0;
      if(rec.id===token) local+=18;
      if(rec.titleNorm.indexOf(token)>=0) local+=8;
      if(rec.partNorm.indexOf(token)>=0) local+=4;
      local+=Math.min(countTokenHits(rec.textNorm,token),4);
      if(local){ matched.push(token); score+=local; }
    });
    if(opts&&opts.activeId===rec.id) score+=16;
    if(opts&&opts.relatedIds&&opts.relatedIds.indexOf(rec.id)>=0) score+=7;
    return { score:score, matched:uniqueTokens(matched) };
  }
  function getTopSections(query,opts){
    var idx=ensurePmnSearchIndex();
    var phraseNorm=normalizeSearchText(query);
    var tokens=tokenizeSearch(query);
    var scored=idx.sections.map(function(rec){
      var res=scoreSectionRecord(rec,tokens,phraseNorm,opts||{});
      return {
        score:res.score,
        matched:res.matched,
        rec:rec
    ...
    scored.sort(function(a,b){
      ...
      return a.rec.id.localeCompare(b.rec.id,undefined,{numeric:true,sensitivity:'base'});
    });
    var seen={}, out=[];
    scored.forEach(function(item){
      if(out.length>=(opts&&opts.maxSections||4)) return;
      if(seen[item.rec.id]) return;
      seen[item.rec.id]=1;
      out.push(item);
    });
    ((opts&&opts.fallbackIds)||[]).forEach(function(id){
      if(out.length>=(opts&&opts.maxSections||4)) return;
      if(seen[id]||!idx.sectionById[id]) return;
      seen[id]=1;
      out.push({ score:0, matched:[], rec:idx.sectionById[id] });
    });
    return out;
  }
  function scoreGlossaryRecord(rec,tokens,phraseNorm,sections){
    var score=0, matched=[];
    if(phraseNorm){
      if(rec.termNorm.indexOf(phraseNorm)>=0) score+=16;
      if(rec.defNorm.indexOf(phraseNorm)>=0) score+=7;
    }
    tokens.forEach(function(token){
      var local=0;
      if(rec.termNorm.indexOf(token)>=0) local+=8;
      local+=Math.min(countTokenHits(rec.defNorm,token),2);
      if(local){ matched.push(token); score+=local; }
    });
    (sections||[]).forEach(function(section){
      if(section && section.rec && rec.termNorm && section.rec.textNorm.indexOf(rec.termNorm)>=0) score+=4;
      if(section && section.rec && rec.termNorm && section.rec.titleNorm.indexOf(rec.termNorm)>=0) score+=5;
    });
    return { score:score, matched:uniqueTokens(matched) };
  }
  function getTopGlossary(query,sections,maxGlossary){
    var idx=ensurePmnSearchIndex();
    var phraseNorm=normalizeSearchText(query);
    var tokens=tokenizeSearch(query);
    var scored=idx.glossary.map(function(rec){
      var res=scoreGlossaryRecord(rec,tokens,phraseNorm,sections);
      return { score:res.score, matched:res.matched, rec:rec };
    }).filter(function(item){ return item.score>0; });
    scored.sort(function(a,b){
      if(b.score!==a.score) return b.score-a.score;
      return a.rec.term.localeCompare(b.rec.term,undefined,{numeric:true,sensitivity:'base'});
    });
    return scored.slice(0,maxGlossary||5);
  }
  function buildContextPack(query,opts){
    opts=opts||{};
    var combinedQuery=[query||'', opts.querySeed||''].join(' ').trim();
    var sections=getTopSections(combinedQuery,opts);
    var glossary=getTopGlossary(combinedQuery,sections,opts.maxGlossary||5);
    return {
      query:combinedQuery,
      sections:sections,
      glossary:glossary
  ...
  function formatSectionPacket(sections){
    return sections.map(function(item,idx){
      var bits=[
        (idx+1)+'. '+item.rec.id+' - '+item.rec.title,
        'Part: '+item.rec.part
      ];
      if(item.matched&&item.matched.length) bits.push('Matched: '+item.matched.join(', '));
      bits.push('Excerpt: '+truncateText(item.rec.excerpt,540));
      return bits.join('\n');
    }).join('\n\n');
  }
  function formatGlossaryPacket(glossary){
    if(!glossary.length) return 'None surfaced by the local PMN glossary retrieval.';
    return glossary.map(function(item,idx){
      return (idx+1)+'. '+item.rec.term+' ['+item.rec.category+'] - '+truncateText(item.rec.definition,240);
    }).join('\n');
  }
  ...
  function providerUrl(target){
    return target==='gemini' ? 'https://gemini.google.com/app' : 'https://chatgpt.com/';
  }
  function providerLabel(target){
    return target==='gemini' ? 'Gemini' : 'ChatGPT';
  }
  function findPartIndex(partLabel){
    for(var i=0;i<PARTS.length;i++){
      if(String(PARTS[i].part)===String(partLabel)) return i;
    }
    return -1;
  }
  function openPartStart(partLabel,opts){
    var idx=findPartIndex(partLabel);
    if(idx>=0){ jumpToSection(idx,0,opts); return true; }
    nav('srch'); showContentsPanel();
    return false;
  }
  ...
  function buildReaderAgentPrompt(question,pack){
    var p=PARTS[curP], s=p.subs[curS];
    var relatedIds=(REL[s.id]||[]).slice(0,4);
    var related=relatedIds.map(function(r){
      var info=LOOK[r];
      return info ? (r+' - '+info.title) : r;
    });
    pack=pack||buildContextPack(question,{
      activeId:s.id,
      relatedIds:relatedIds,
      fallbackIds:[s.id].concat(relatedIds.slice(0,2)),
      maxSections:4,
      maxGlossary:5,
      querySeed:[s.id,s.title,p.title].join(' ')
    });
    return [
      'You are acting as PMN Agent for Progressive Materialist Naturalism (PMN) by Nova Dharma.',
      'Use the supplied PMN context pack as the primary source.',
      'When something is explicit in the PMN material, say so and cite the section ID.',
      'When you infer beyond the text, label it as inference.',
      'Do not assume you can reliably reconstruct the manuscript from a URL or page anchor alone.',
      'Prefer concise but rigorous answers.',
      '',
      'USER QUESTION:',
      question||'Explain this section and its role in the wider PMN architecture.',
      '',
      'ACTIVE SECTION:',
      s.id+' - '+s.title,
      'PART:',
      partDisplay(p),
      '',
      'RETRIEVED PMN PASSAGES:',
      formatSectionPacket(pack.sections),
      '',
      'RELATED SECTIONS:',
      related.length?related.join('\n'):'None surfaced by the current relation map.',
      '',
      'GLOSSARY TERMS:',
      formatGlossaryPacket(pack.glossary),
      '',
      'RESPONSE GOAL:',
      'Answer the user from PMN, connect to adjacent sections when helpful, and suggest where to read next if the answer depends on surrounding architecture.'
    ].join('\n');
  }
  function buildHomeAgentPrompt(question,pack){
    pack=pack||buildContextPack(question,{
      fallbackIds:['how-to-read-this-document','3.0','6.1','10.1','15.15','17.0'],
      maxSections:5,
      maxGlossary:6,
      querySeed:'PMN orientation manuscript architecture foundations power formula cases glossary'
    });
    return [
      'You are acting as PMN Agent for Progressive Materialist Naturalism (PMN) by Nova Dharma.',
      'Use the supplied PMN context pack as the primary source.',
      'When something is explicit in PMN, say so. When you infer, label it as inference.',
      'Treat this as a reader asking for help inside the PMN manuscript.',
      'Do not assume you can reliably browse the PMN site by URL or anchor alone.',
      '',
      'USER QUESTION:',
      question||'Give me a strong orientation to PMN and tell me where to start.',
      '',
      'RETRIEVED PMN PASSAGES:',
      formatSectionPacket(pack.sections),
      '',
      ...
      '',
      'RESPONSE GOAL:',
      'Answer as a PMN guide, name the most relevant parts or sections, and point the reader to the best next step in the manuscript.'
    ].join('\n');
  }
  function updateReaderAgentStatus(){
    ...
    var retrieved=pack.sections.map(function(item){ return item.rec.id; }).join(', ');
    var terms=pack.glossary.map(function(item){ return item.rec.term; }).join(', ');
    var msg='Current packet: '+s.id+' - '+s.title+'\nPart: '+partDisplay(p);
    if(retrieved) msg+='\nRetrieved passages: '+retrieved;
    if(terms) msg+='\nGlossary: '+terms;
    setStatus('reader-agent-status', msg);
  }
  function launchAgent(target, source, question, copyOnly){
    var pack=source==='reader'
      ? buildContextPack(question,{
          activeId:PARTS[curP].subs[curS].id,
          relatedIds:(REL[PARTS[curP].subs[curS].id]||[]).slice(0,4),
          fallbackIds:[PARTS[curP].subs[curS].id].concat((REL[PARTS[curP].subs[curS].id]||[]).slice(0,2)),
          maxSections:4,
          maxGlossary:5,
          querySeed:[PARTS[curP].subs[curS].id,PARTS[curP].subs[curS].title,PARTS[curP].title].join(' ')
        })
      : buildContextPack(question,{
          fallbackIds:['how-to-read-this-document','3.0','6.1','10.1','15.15','17.0'],
          maxSections:5,
    ...
    var prompt=source==='reader' ? buildReaderAgentPrompt(question,pack) : buildHomeAgentPrompt(question,pack);
    var dest=providerLabel(target);
    var win=null;
    var refs=pack.sections.map(function(item){ return item.rec.id; }).join(', ');
    var gloss=pack.glossary.map(function(item){ return item.rec.term; }).slice(0,3).join(', ');
    if(!copyOnly){
      try{ win=window.open(providerUrl(target),'_blank','noopener'); }catch(e){}
    }
    copyText(prompt).then(function(){
      if(source==='reader'){
        setStatus('reader-agent-status', copyOnly
          ? 'Grounded prompt copied with '+pack.sections.length+' PMN passages. Paste it into any AI workspace.'
          : 'Grounded prompt copied. '+dest+' opened in a new tab with '+pack.sections.length+' PMN passages ready to paste.'
        );
      } else {
        setStatus('home-ai-terminal', copyOnly
          ? '>> GROUNDED PROMPT COPIED.\n>> RETRIEVED SECTIONS: '+(refs||'NONE')+'.\n>> PASTE IT INTO ANY AI WORKSPACE.'
          : '>> GROUNDED PROMPT COPIED.\n>> '+dest.toUpperCase()+' OPENED IN A NEW TAB.\n>> RETRIEVED SECTIONS: '+(refs||'NONE')+'.\n>> PASTE TO CONTINUE WITH PMN CONTEXT.'
        );
        if(gloss){
          setStatus('home-ai-terminal', g('home-ai-terminal').textContent+'\n>> GLOSSARY: '+gloss+'.');
        }
      }
      if(source==='home'&&g('home-ai-input')) g('home-ai-input').value='';
    }).catch(function(){
      if(source==='reader'){
        setStatus('reader-agent-status','Copy failed in-browser. Select and copy manually after reopening the handoff.');
      } else {
        setStatus('home-ai-terminal','>> COPY FAILED IN BROWSER.\n>> TRY AGAIN OR COPY MANUALLY AFTER REOPENING THE HANDOFF.');
      }
    });
    if(!copyOnly&&source==='home'&&!win){
      setStatus('home-ai-terminal','>> PROMPT COPIED IF ALLOWED.\n>> POP-UP BLOCKED, SO OPEN '+dest.toUpperCase()+' MANUALLY AND PASTE.');
    }
  }
  ...
  function upgradeHomeAi(){
    ...
    inner.innerHTML=
      ...
      +'<button id="home-ai-btn" class="home-ai-btn" type="button">Open handoff</button>'
      +'</div>'
      +'<div class="home-ai-terminal" id="home-ai-terminal">&gt;&gt; PMN AGENT READY\n&gt;&gt; HOME MODE PACKAGES A MANUSCRIPT-LEVEL CONTEXT SUMMARY.\n&gt;&gt; ASK A QUESTION, THEN OPEN CHATGPT OR GEMINI.</div>'
      +'<div class="home-ai-actions"><button id="home-ai-copy" class="pmn-agent-btn" type="button">Copy prompt only</button></div>'
      +'<p class="home-ai-note">This avoids a private API requirement. The site prepares PMN context locally, copies it, and hands you off to your own AI workspace.</p>';
  }
...
(function(){
  ...
  function callClaude(messages, sysPrompt, onDone, onError) {
    var msg='Inline provider mode is parked in this static build. Use the ChatGPT/Gemini handoff with PMN context packs, or connect a secure backend before re-enabling in-page answers.';
    if(onError) onError(msg);
    else if(onDone) onDone(msg);
  }
  ...
  setTimeout(function(){
    ensureData();
    window._pmnParts=PARTS;
    window._pmnRel=REL;
    // Patch openSec to expose curP/curS and fire hooks
    if(typeof openSec==='function'&&!window._pmnOpenSecPatched){
      window._pmnOpenSecPatched=true;
      var _orig=openSec;
      window.openSec=openSec=function(pi,si,nohash){
        window._pmnCurP=pi;
        window._pmnCurS=si;
        _orig(pi,si,nohash);
        setTimeout(function(){
          var p=PARTS[pi],s=p&&p.subs[si];
          if(s){
...
(function(){
  ...
  // 5. Ctrl+K Search Command Palette
  document.addEventListener('keydown', (e) => {
    if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
      e.preventDefault();
      const searchInput = document.getElementById('srch-part');
      if (searchInput) {
