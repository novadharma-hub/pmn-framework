/* ═══════════════════════════════════════════════════════════════
   @LEGACY — PMN Monolithic App (Vanilla TypeScript)

   File ini HANYA digunakan oleh modularizer.py compile_mode()
   untuk generate index.offline.html (monolithic offline build).

   React SPA (src/) TIDAK menggunakan file ini.
   JANGAN hapus — masih dibutuhkan untuk distribusi offline/arsip.

   Jika suatu saat monolithic build dihentikan sepenuhnya,
   file ini bisa dipindahkan ke /backup/.
   ═══════════════════════════════════════════════════════════════ */
...
interface Window {
  ...
  LOOK: any;
  CITED: any;
  buildContextPack: any;
  ensureData: any;
  stripHtml: any;
  openSec: any;
  _pmnParts: any;
  _pmnRel: any;
  _pmnOpenSecPatched: any;
  _pmnCurP: any;
  _pmnCurS: any;
  _pmnRenderHL: any;
  _pmnClearReaderChat: any;
  _glTimeout: any;
  jumpToSection: any;
  getJumpOrigin: any;
  jumpToSectionById: any;
  nav: any;
  showContentsPanel: any;
  esc: any;
  openPartStart: any;
  resolveSectionId: any;
  linkXrefs: any;
  _afterOpenSec: any;
}
...
declare var GLG: any;
declare var REL: any;
declare var LOOK: any;
declare var CITED: any;
declare var readMap: any;
declare var pip: any;
declare var buildTOC: any;
declare var esc: any;
declare var ensureData: any;
declare var stripHtml: any;
declare var buildContextPack: any;
declare var launchAgent: any;
...
(async function initializeApp() {
  ...
  (function () {
    ...
    function g(id: string): any { return document.getElementById(id); }
    ...
    function openSec(pi, si, nohash?: boolean) {
    ...
    function populatePartFilter() {
      ...
      for (var pi = 0; pi < PARTS.length; pi++) {
        ...
        o.value = String(pi); o.textContent = pshort(PARTS[pi]);
        sel.appendChild(o);
      }
    ...
    // Event delegation
    g('sv-body').addEventListener('click', function (e: any) {
    ...
    g('related-sec').addEventListener('click', function (e: any) {
      var btn = e.target.closest('[data-pi]'); if (!btn) return;
      jumpToSection(+btn.getAttribute('data-pi'), +btn.getAttribute('data-si'), { origin: getJumpOrigin() });
    });
    g('toc-grid').addEventListener('click', function (e: any) {
      var btn = e.target.closest('[data-pi]'); if (!btn) return;
      jumpToSection(+btn.getAttribute('data-pi'), +btn.getAttribute('data-si'), { origin: getJumpOrigin() });
    });
    g('toc-panel').addEventListener('click', function (e: any) {
    ...
    g('sb-list').addEventListener('click', function (e: any) {
      var btn = e.target.closest('[data-pi]'); if (!btn) return;
      jumpToSection(+btn.getAttribute('data-pi'), +btn.getAttribute('data-si'), { origin: getJumpOrigin() });
    });
    ...
    g('suggestions') && g('suggestions').addEventListener('click', function (e: any) {
      var b = e.target.closest('.suggest-btn');
      if (b) { jumpToSection(+b.getAttribute('data-pi'), +b.getAttribute('data-si'), { origin: getJumpOrigin() }); }
    });
    g('home-view') && g('home-view').addEventListener('click', function (e: any) {
...
.pmn-chat-body.thinking{color:var(--mute);font-style:italic;animation:blink 1s infinite}
  ...
  (function () {
    ...
    (function () {
      ...
      // Claude inline
      var chatLog = document.getElementById('home-chat-log') as any;
      var inp = document.getElementById('home-ai-input') as any;
      var btn = document.getElementById('home-ai-btn') as any;
      var clearBtn = document.getElementById('home-chat-clear') as any;
      var exportBtn = document.getElementById('home-ai-export') as any;
      var history = [];

      function appendMsg(role, text, thinking?: boolean) {
      ...
      function setupRedirect(qId, btnId, cpyId, statId, modeId, url) {
        var qEl = document.getElementById(qId) as any, rbtn = document.getElementById(btnId), cpyBtn = document.getElementById(cpyId), stat = document.getElementById(statId), modeSel = document.getElementById(modeId) as any;
        if (!rbtn) return;
        function go(openWin) {
    ...
    (function () {
      ...
      var chatLog = document.getElementById('pmn-chat-log') as any;
      var chatIn = document.getElementById('pmn-chat-in') as any;
      var sendBtn = document.getElementById('pmn-chat-send') as any;
      var clearBtn = document.getElementById('pmn-chat-clear') as any;
      var rHistory = [];
    ...
    (function () {
      ...
      function showToolbar(x, y, noteVal?) {
        ...
        (document.getElementById('hl-toolbar-note-in') as any).value = noteVal || '';
      ...
      toolbar.querySelectorAll('.hl-btn').forEach(function (btn) {
        btn.addEventListener('click', function (e) {
          ...
          var note = (document.getElementById('hl-toolbar-note-in') as any).value.trim();
          if (editHlId) {
            var secId = getSecId();
      ...
      document.getElementById('hl-save-note-btn').addEventListener('click', function (e) {
        ...
        if (editHlId && secId) {
          ...
          var sp = document.querySelector('[data-hl-id="' + editHlId + '"]') as HTMLElement;
    ...
    // ──────────────────────────────────────────────────────────────────
    (function () {
      var srchIn = document.getElementById('srch-in') as any;
  ...
  (function () {
    ...
    function getHero() { return document.querySelector('.hero') as HTMLElement; }
    ...
    (function () {
      ...
      var input = backdrop.querySelector('.cmd-palette-input') as HTMLInputElement;
      var results = backdrop.querySelector('.cmd-palette-results') as HTMLElement;
      ...
      results.addEventListener('click', function (e: any) {
